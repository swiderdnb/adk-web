...
import {fakeAsync,
        initTestBed,
        tick} from '../../../testing/utils.google';
...
describe('TraceEventComponent', () => {
  ...
  describe('when initialized', () => {
    it.autoTick('should populate eventData from the trace service', () => {
    ...
    it.autoTick('should populate selectedRow from the trace service', () => {
      expect(component.selectedRow).toEqual(span);
    });

    it.autoTick('should call event service to get event details for the selected row',
       () => {
    ...
    it.autoTick('should parse LLM request from the selected span attributes', () => {
    ...
    it.autoTick('should parse LLM response from the selected span attributes', () => {
      traceService.selectedTraceRow$.next({
        ...span,
        attributes: {
          'gcp.vertex.agent.event_id': EVENT_ID,
  ...
  describe('getEventIdFromSpan()', () => {
    it.autoTick('should return the event_id from the selected row attributes', () => {
      expect(component.getEventIdFromSpan()).toEqual(EVENT_ID);
    });

    it.autoTick('should return undefined if no row is selected', () => {
      component.selectedRow = undefined;
      expect(component.getEventIdFromSpan()).toBeUndefined();
    });

    it.autoTick('should return undefined if the selected row lacks the event_id attribute',
       () => {
  ...
  describe('getEventDetails()', () => {
    it.autoTick('should return event details for the selected row from eventData',
       () => {
  ...
  describe('closePanel()', () => {
    it.autoTick('should emit panelClosed event when called', () => {
  ...
  describe('openViewImageDialog()', () => {
    it.autoTick('should open the ViewImageDialogComponent with the provided image data',
       () => {
  ...
  describe('getEventGraph()', () => {
    describe('when event service returns no dotSrc', () => {
      ...
      it.autoTick('should set renderedEventGraph to undefined', fakeAsync(() => {
    ...
    describe('when event service returns dotSrc', () => {
      ...
      it.autoTick('should render SVG using GraphService', fakeAsync(() => {
           component.getEventGraph(EVENT_ID);
           tick();
      ...
      it.autoTick('should sanitize the result of graph rendering', fakeAsync(() => {
           component.getEventGraph(EVENT_ID);
           tick();
