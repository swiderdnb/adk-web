...
describe('StreamChatService', () => {
  ...
  describe('startAudioChat', () => {
    it.autoTick('should connect to the WebSocket server', async () => {
    ...
    it.autoTick('should start audio recording', async () => {
      await service.startAudioChat({
        appName: 'fake-app-name',
        userId: 'fake-user-id',
        sessionId: 'fake-session-id'
    ...
    it.autoTick('should start to send buffered audio periodically', fakeAsync(() => {
  ...
  describe('stopAudioChat', () => {
    it.autoTick('should stop audio recording', () => {
      service.stopAudioChat();

      expect(mockAudioRecordingService.stopRecording).toHaveBeenCalled();
    });

    it.autoTick('should close WebSocket connection', () => {
      service.stopAudioChat();

      expect(mockWebSocketService.closeConnection).toHaveBeenCalled();
    });

    it.autoTick('should clear the stream to send buffered audio periodically',
       fakeAsync(() => {
         mockAudioRecordingService.getCombinedAudioBuffer.and.returnValue(
             Uint8Array.of());
         service.startAudioChat({
  ...
  describe('startVideoChat', () => {
    ...
    it.autoTick('should start video recording', async () => {
      await service.startVideoChat({
        appName: 'fake-app-name',
        userId: 'fake-user-id',
        sessionId: 'fake-session-id',
    ...
    it.autoTick('should start to send video frame periodically', fakeAsync(() => {
  ...
  describe('stopVideoChat', () => {
    ...
    it.autoTick('should stop video recording', () => {
      service.stopVideoChat(videoContainer);
    ...
    it.autoTick('should clear the stream to send video frame periodically',
       fakeAsync(() => {
         mockVideoService.getCapturedFrame.and.resolveTo(Uint8Array.of());
         service.startVideoChat({
           appName: 'fake-app-name',
