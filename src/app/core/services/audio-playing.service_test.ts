...
describe('AudioPlayingService', () => {
  ...
  describe('playAudio', () => {
    it.autoTick('should not play audio if the buffer is empty', () => {
      spyOn<any>(service, 'playPCM').and.callThrough();
      service.playAudio([]);
      expect((service as any).playPCM).not.toHaveBeenCalled();
    });

    it.autoTick('should play audio if the buffer is not empty', () => {
      spyOn<any>(service, 'playPCM').and.callThrough();
    ...
    it.autoTick('should combine the buffers and play the combined audio', () => {
      spyOn<any>(service, 'playPCM').and.callThrough();
      const buffer = [new Uint8Array([1, 2]), new Uint8Array([3, 4])];
    ...
    it.autoTick('should schedule audio sources correctly', () => {
    ...
    it.autoTick('removes source from schedule after playback finishes', () => {
      const mockSource = createMockSource();
      mockAudioContext.createBufferSource.and.returnValue(mockSource);
  ...
  describe('stopAudio', () => {
    it.autoTick('should stop all scheduled sources', () => {
    ...
    it.autoTick('should reset scheduling time so next audio plays immediately', () => {
      const mockSource1 = createMockSource();
      const mockSource2 = createMockSource();
      mockAudioContext.createBufferSource.and.returnValues(
          mockSource1, mockSource2);
