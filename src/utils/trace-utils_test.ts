...
describe('trace-utils', () => {
  describe('normalizeSpan', () => {
    it.autoTick('should keep existing llm_request and llm_response', () => {
      const span: Span = {
        name: 'test',
        start_time: 0,
        end_time: 100,
    ...
    it.autoTick('should extract input and output for execute_tool spans from attributes', () => {
      const span: Span = {
    ...
    it.autoTick('should extract input from logs for non-execute_tool spans', () => {
      const logs: Log[] = [
        {
    ...
    it.autoTick('should ignore user messages with function responses', () => {
      const logs: Log[] = [{
        event_name: 'gen_ai.user.message',
        body: JSON.stringify({
    ...
    it.autoTick('should extract output from logs for non-execute_tool spans', () => {
      const logs: Log[] = [
        {
    ...
    it.autoTick('should handle missing logs gracefully', () => {
      const span: Span = {
        name: 'generate_content',
        start_time: 0,
        end_time: 100,
    ...
    it.autoTick('should handle missing attributes gracefully', () => {
      const span: Span = {
        name: 'generate_content',
        start_time: 0,
        end_time: 100,
  ...
  describe('normalizeEventTelemetry', () => {
    ...
    it.autoTick('should extract input and output from telemetry properties if present', () => {
      const telemetry: any = {
        'gcp.vertex.agent.tool_call_args': '{"arg": "val"}',
        'gcp.vertex.agent.tool_response': '{"result": "ok"}'
      };
    ...
    it.autoTick('should extract from logs if attributes are missing', () => {
      const logs: Log[] = [
        {
          event_name: 'gen_ai.choice',
          body: 'log response',
